...
public class ChatActivity extends AppCompatActivity implements Handler.Callback,
    ...
    public void MessageEventBus(final MessageInfo messageInfo) {
        ...
        if(!flag) {
            if (messageInfo.getRole() == Constants.RECEIVER) {
                if (messageInfo.getType() == Constants.PRIVATE_CHAT_REQUEST) {
                    PeerInfo peer = messageInfo.getFrom();
                ...
                } else if (messageInfo.getType() == Constants.BROAD_PEER) {
                    peerInfoArrayList = gson.fromJson(messageInfo.getContent(), new TypeToken<ArrayList<PeerInfo>>() {
                    }.getType());
                } else {
                    MessageInfo showReceivedMessage = messageInfo;
                    showReceivedMessage.setType(Constants.CHAT_ITEM_TYPE_LEFT);
                    showReceivedMessage.setContent("From " + messageInfo.getFrom().getSipAddress() + "\n" + messageInfo.getContent());
                    messageInfos.add(messageInfo);
