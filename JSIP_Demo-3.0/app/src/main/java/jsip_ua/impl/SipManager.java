...
import jsip_ua.impl.sip_msg.Message;
import jsip_ua.impl.sip_msg.SipRequestMsgBuilder;

import static com.rance.chatui.util.Constants.flag;
...
//			Request r = inviteRequest.buildMessage(this, to, message);
			Request r = sipRequestBuilder.buildMessage(this, to, message);

			final ClientTransaction transaction = this.sipProvider.getNewClientTransaction(r);
			Thread thread = new Thread() {
				public void run() {
					try {
						transaction.setRetransmitTimer(5000);
						System.out.println(transaction.getRetransmitTimer());
						transaction.sendRequest();
					} catch (SipException e) {
						e.printStackTrace();
					}
	...
	public void processRequest(RequestEvent arg0) {
		...
		if (request.getMethod().equals("MESSAGE")) {
			...
			try {
				...
				dispatchSipEvent(new SipEvent(this, SipEventType.MESSAGE, message,
	...
	public void processResponse(ResponseEvent arg0) {
		...
		if (response.getStatusCode() == Response.PROXY_AUTHENTICATION_REQUIRED
			...
			AuthenticationHelper authenticationHelper = ((SipStackExt) sipStack)
					.getAuthenticationHelper(new AccountManagerImpl(sipProfile.getSipUserName(),
							sipProfile.getRemoteIp(), sipProfile.getSipPassword()), headerFactory);
			try {
	...
	private void sendOk(RequestEvent requestEvt) {
        Request req = requestEvt.getRequest();
        FromHeader from = (FromHeader) req.getHeader("From");
        String content = new String(req.getRawContent());
        MessageInfo msg = gson.fromJson(content, MessageInfo.class);
        msg.setRole(Constants.RECEIVER);
        if(flag){
        	msg.setSender(from.getAddress().toString());
		}
        //String message = "From " + from.getAddress().toString() + "|" + content;
		EventBus.getDefault().post(msg);
        //handler.obtainMessage(Constants.MESSAGE_READ, message).sendToTarget();
		Response response;
		try {
